...
<thead>
    <th style="text-align: left; width: 33%;">Year: {{$year->start_year}} - {{$year->end_year}}</th>
    <th style="font-weight: bold; font-size: 25px; text-align: centre;">{{$room_name->name}}</th>
    <th style="font-weight: bold; width:33%; text-align: right;">Session:{{$session_duration_mast[$session_id]}}</th>
</thead>
</table>
                ...
                @foreach ($workingHours as $hour)
                    ...
                    <td style="font-size: 12px; width: {{$workingwidth}}%; max-width: 100px; word-wrap: break-word; white-space: normal; overflow: hidden; max-lines: 2;"">
                        ...
                        ?>
                                ...
                                <!-- <span style="font-weight: bold;">{{ $course }}</span><br> -->
                                {{ $subject }}  ({{ $lecture }})<br>
                                {{ $faculty }}<br>
